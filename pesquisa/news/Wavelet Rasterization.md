...
Figure 1: Slices from a 3D rasterization of the Happy Buddha statue computed on a 643
grid to illustrate the anti-aliased
...
inclusion. Other algorithms [FL00, HW02, DCB∗
04, ED06,
...
c 2011 The Eurographics Association and Blackwell Publishing Ltd.
J. Manson & S. Schaefer / Wavelet Rasterization
Figure 2: The 2d Haar basis functions. Each function is shown over the domain [0,1)
2
and is piecewise constant (−1/+1) on
each quadrant. Translations of Ψ¯
(0,0)
give the low-resolution representation of the function, while scalings and translations of
the functions Ψ¯
(1,0)
, Ψ¯
(0,1)
...
where p = (px, py). Every function g that is locally integrable on R
2
has the wavelet expansion
g(p) = ∑
k∈Z2
c
(0,0)
...
j,k
(p)· n(p) dσ (6)
where F = (fx, fy) is a vector-valued function on R
2
, n(p)
is the outward unit normal to the curve ∂M at point p, and
dσ is the differential length of ∂M. By finding functions F
e
j,k
that satisfy ∇ ·F
e
...
e
j,k
, we can calculate the wavelet coefficients of χM using only the polygon boundary in the line
integrals
c
e
j,k = ∑
...
th polynomial segment of the boundary (line segment for polygons).
...
We make a few simplifying assumptions to facilitate analysis. First we calculate basis functions over the [0,1)
2
domain (the support of the 2D Haar basis) by translating the
input edges by −k and scaling by 2j
. Furthermore, we clip
edges to this domain because the support of the wavelet
functions is only [0,1)
2
. Note that we only need to clip the
...
ey
(py).
There are many possible function F
e
in 2D that satisfy
...
images were all rendered at 5122
pixels on a side. When
...
The c
(0,0)
coefficient is special because it exists only for the
root node of the quadtree and gives the value that is refined
by all other wavelet coefficients. The c
(0,0)
coefficient also
...
(0,0)
calculate the difference between a cell and its sub-cells at the next higher
...
coefficient (c
(0,1)
follows in a similar manner). Again, we could choose α = β =
1
2
to give
F
...
the divergence theorem, F
(1,0)
has infinite support because
Φ¯ has support of [0,∞). We only want to use detail functions
...
of Ψ¯ is finite and is [0,1). By choosing α = 1, β = 0 for
F
(1,0)
and α = 0, β = 1 for F
(0,1)
, we obtain the compactly
supported functions
F
(1,0)
(p) = (Ψ¯ (px),0)
...
The functions F
e
are piecewise-linear because Ψ¯ is
piecewise-linear. However, each quadrant is linear, so we
evaluate these integrals by splitting each polygon edge Pi
into the quadrants and transforming the domains of the quadrants to be [0,1)
2
. We then add the contribution of each split
...
c 2011 The Author(s)
Journal compilation
c 2011 The Eurographics Association and Blackwell Publishing Lt
J. Manson & S. Schaefer / Wavelet Rasterization
...
(t) and F
e
(Pi(t)) are also polynomial. The definite integral is then easy to evaluate and results in an expression
that is quadratic in the Bézier control points. Like in Section 2.1, F
e
is piecewise-polynomial, and we can compute
the coefficients c
e
using the same formula in Equation 8 of
...
ez
(pz).
To satisfy Equation 6, we find functions F
e whose divergences are equal to ψ
e
. These functions have the general
form α+β+γ = 1,
F
e
(p) =
...
we label Qi, j,k
. We give the closed-form solution to the
...
at 10242
resolution in 50.2ms on the GPU with 16xQ antialiasing, 36.8ms with AGG, and 107ms with our method.
...
of spirals (10,000 arms in a 5122
image) is far greater than
...
head and the Eurographics logo using an anti-aliased rasterization of each model on a 10243
grid with our method.
...
we rasterized at a resolution of 40963
. Storing one byte per
...
Table 1: Time taken (in seconds) to rasterize volumes of increasing complexity at 2563
and 40963
. We show the time
...
using a binary rasterization, which is typical of other methods [CC95, FT97, FL00, HW02, DCB∗
04, ED06, ZCEP07,
ED08], and our anti-aliased rasterization over a 2563
grid.
...
2563
grid contoured with Marching Cubes. The left shows
...
J. Manson & S. Schaefer / Wavelet Rasterization
References
[ACSTD07] ALLIEZ P., COHEN-STEINER D., TONG Y., DESBRUN M.: Voronoi-based variational reconstruction of unoriented point sets. In Symposium on Geometry Processing (2007),
pp. 39–48. 3
[BA05] BÆRENTZEN J. A., AANÆS H.: Signed distance computation using the angle weighted pseudonormal. IEEE Transactions on Visualization and Computer Graphics 11 (2005), 243–
253. 3
[Cat78] CATMULL E.: A hidden-surface algorithm with antialiasing. SIGGRAPH Comput. Graph. 12 (1978), 6–11. 2
[CC95] CARVALHO P. C. P., CAVALCANTI P. R.: Graphics
Gems V. AP Professional, Chestnut Hill, MA, 1995, ch. Point
in Polyhedron Testing Using Spherical Polygons, pp. 42–49. 3, 8
[CK10] CAMPEN M., KOBBELT L.: Exact and robust (self-) intersections for polygonal meshes. Computer Graphics Forum
...
[Cro77] CROW F. C.: The aliasing problem in computergenerated shaded images. Commun. ACM 20 (1977), 799–805.
1
[DCB∗04] DONG Z., CHEN W., BAO H., ZHANG H., PENG Q.:
...
[Duf89] DUFF T.: Polygon scan conversion by exact convolution.
...
[HE93] HERSCH R. D., (ED.): Font rasterization, the state of art.
In Visual and Technical Aspects of Type. Cambridge University
Press, 1993, pp. 78–109. 2
[HW02] HAUMONT D., WARZÉE N.: Complete polygonal scene
...
[JC99] JOUPPI N. P., CHANG C.-F.: Z3: An economical hardware technique for high-quality antialiasing and transparency.
In SIGGRAPH/EUROGRAPHICS Workshop on Graphics Hardware (1999), pp. 85–93. 2
[Kal07] KALLIO K.: Scanline edge-flag algorithm for antialiasing. In Theory and Practice of Computer Graphics (2007),
pp. 81–88. 2
[Kaz05] KAZHDAN M.: Reconstruction of solid models from oriented point sets. In Symposium on Geometry Processing (2005),
pp. 73–82. 3
[KBH06] KAZHDAN M., BOLITHO M., HOPPE H.: Poisson
surface reconstruction. In Symposium on Geometry Processing
(2006), pp. 61–70. 3
[LB05] LOOP C., BLINN J.: Resolution independent curve rendering using programmable graphics hardware. Transactions on
...
[McC95] MCCOOL M. D.: Analytic antialiasing with prism
splines. In SIGGRAPH (1995), pp. 429–436. 2
[MN88] MITCHELL D. P., NETRAVALI A. N.: Reconstruction
...
[NH08] NEHAB D., HOPPE H.: Random-access rendering of
...
[TWGT10] THÜREY N., WOJTAN C., GROSS M., TURK G.: A
multiscale approach to mesh-based surface tension flows. SIGGRAPH 29, 3 (2010), 48:1–10. 2
[ZCEP07] ZHANG L., CHEN W., EBERT D., PENG Q.: Conservative voxelization. Visual Computer 23, 9 (2007), 783–792. 3,
8
Appendix A: Polygon Coefficient Calculation
Before scaling edges, calculate the c
(0,0)
coefficient as
c
(0,0) +=
...
c 2011 The Eurographics Association and Blackwell Publishing Ltd.
J. Manson & S. Schaefer / Wavelet Rasterization
Note that we only compute the c
(0,0)
coefficient for the root
...
c
(1,0) += Lx(Q0,0) +Lx(Q0,1) +Kx(Q1,0)
−Lx(Q1,0) +Kx(Q1,1) − Lx(Q1,1)
c
(0,1) += Ly(Q0,0) +Ly(Q1,0) +Ky(Q0,1)
−Ly(Q0,1) +Ky(Q1,1) − Ly(Q1,1)
c
(1,1) += Lx(Q0,0) − Lx(Q0,1) +Kx(Q1,0)
−Lx(Q1,0) − Kx(Q1,1) +Lx(Q1,1).
(8)
...
points (v0, v1, v2). Then c
(0,0)
can still be written in terms of
determinants
c
(0,0) +=
1
...
(v2,x − v0,x)
Lx(Q) = 1
24 (3v0,xv0,y +2v0,yv1,x − 2v0,xv1,y +v0,yv2,x
+2v1,yv2,x − (v0,x +2v1,x +3v2,x)v2,y)
Ly(Q) = 1
24 (2v1,yv2,x +v0,y(2v1,x +v2,x) − 2v1,xv2,y
+3v2,xv2,y − v0,x(3v0,y +2v1,y +v2,y)).
...
(v3,x − v0,x)
Lx(Q) = 1
80 (6v2,yv3,x +3v1,y(v2,x +v3,x)
+v0,y(6v1,x +3v2,x +v3,x)
+v0,x(10v0,y − 6v1,y − 3v2,y − v3,y)
− 6v2,xv3,y − 10v3,xv3,y − 3v1,x(v2,y +v3,y))
Ly(Q) = 1
80 (6v2,yv3,x +3v1,y(v2,x +v3,x)
+v0,y(6v1,x +3v2,x +v3,x)
− 6v2,xv3,y +10v3,xv3,y − 3v1,x(v2,y +v3,y)
...
Kx(Q) = 1
16 (−v1,zv2,y +v0,z(−v1,y +v2,y)
+v0,y(v1,z − v2,z) +v1,yv2,z)
Ky(Q) = 1
16 (v0,z(v1,x − v2,x) +v1,zv2,x
− v1,xv2,z +v0,x(−v1,z +v2,z))
Kz(Q) = 1
16 (−v1,yv2,x +v0,y(−v1,x +v2,x)
+v0,x(v1,y − v2,y) +v1,xv2,y)
Lx(Q) = −1
48 (v0,x +v1,x +v2,x)(v0,z(v1,y − v2,y)
+v1,zv2,y − v1,yv2,z +v0,y(−v1,z +v2,z))
Ly(Q) = 1
48 (v0,y +v1,y +v2,y)(v0,z(v1,x − v2,x)
+v1,zv2,x − v1,xv2,z +v0,x(−v1,z +v2,z))
Lz(Q) = −1
48 (v0,y(v1,x − v2,x) +v1,yv2,x
− v1,xv2,y +v0,x(−v1,y +v2,y))(v0,z +v1,z +v2,z).
Assuming that Qi, j,k
represents the portion of the polygon
(triangulated) in the i jk octant with vertices v0, v1, v2, we
can then compute the wavelet coefficients using
c
(1,0,0) += Lx(Q0,0,0) +Lx(Q0,0,1) +Lx(Q0,1,0) +Lx(Q0,1,1)
+Kx(Q1,0,0) − Lx(Q1,0,0) +Kx(Q1,0,1) − Lx(Q1,0,1)
+Kx(Q1,1,0) − Lx(Q1,1,0) +Kx(Q1,1,1) − Lx(Q1,1,1)
c
(0,1,0) += Lx(Q0,0,0) +Lx(Q0,0,1) +Lx(Q1,0,0) +Lx(Q1,0,1)
+Kx(Q0,1,0) − Lx(Q0,1,0) +Kx(Q0,1,1) − Lx(Q0,1,1)
+Kx(Q1,1,0) − Lx(Q1,1,0) +Kx(Q1,1,1) − Lx(Q1,1,1)
c
(0,0,1) += Lx(Q0,0,0) +Lx(Q1,0,0) +Lx(Q0,1,0) +Lx(Q1,1,0)
+Kx(Q0,0,1) − Lx(Q0,0,1) +Kx(Q1,0,1) − Lx(Q1,0,1)
+Kx(Q0,1,1) − Lx(Q0,1,1) +Kx(Q1,1,1) − Lx(Q1,1,1)
c
(1,1,0) += Lx(Q0,0,0) +Lx(Q0,0,1) − Lx(Q0,1,0) − Lx(Q0,1,1)
+Kx(Q1,0,0) − Lx(Q1,0,0) +Kx(Q1,0,1) − Lx(Q1,0,1)
−Kx(Q1,1,0) +Lx(Q1,1,0) − Kx(Q1,1,1) +Lx(Q1,1,1)
c
(1,0,1) += Lx(Q0,0,0) − Lx(Q0,0,1) +Lx(Q0,1,0) − Lx(Q0,1,1)
+Kx(Q1,0,0) − Lx(Q1,0,0) − Kx(Q1,0,1) +Lx(Q1,0,1)
+Kx(Q1,1,0) − Lx(Q1,1,0) − Kx(Q1,1,1) +Lx(Q1,1,1)
c
(0,1,1) += Lx(Q0,0,0) − Lx(Q0,0,1) +Lx(Q1,0,0) − Lx(Q1,0,1)
+Kx(Q0,1,0) − Lx(Q0,1,0) − Kx(Q0,1,1) +Lx(Q0,1,1)
+Kx(Q1,1,0) − Lx(Q1,1,0) − Kx(Q1,1,1) +Lx(Q1,1,1)
c
(1,1,1) += Lx(Q0,0,0) − Lx(Q0,0,1) − Lx(Q0,1,0) +Lx(Q0,1,1)
+Kx(Q1,0,0) − Lx(Q1,0,0) − Kx(Q1,0,1) +Lx(Q1,0,1)
−Kx(Q1,1,0) +Lx(Q1,1,0) +Kx(Q1,1,1) − Lx(Q1,1,1).
